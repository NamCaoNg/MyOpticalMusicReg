...
from src.utils.utils import get_unit_size
from src.utils.logger import get_logger
from src.utils.draw_bbox import BBox, get_center, get_rotated_bbox, to_rgb_img, draw_bounding_boxes
from src.extraction.notehead_extraction import NoteType
from src.utils.morph import morph_open, morph_close

logger = get_logger(__name__)


def _bin(img: ndarray) -> ndarray:
    return (img > 0).astype(np.uint8)


def _clip_box(box: BBox, shape: Tuple[int, int]) -> BBox:
    h, w = shape
    x1, y1, x2, y2 = box
    return (
        max(0, min(int(x1), w)),
...
def _contour_mask(shape: Tuple[int, int], cnt: ndarray) -> ndarray:
    mask = np.zeros(shape, dtype=np.uint8)
    cv2.fillPoly(mask, [cnt.astype(np.int32)], 1)
    return mask
...
def scan_dot(
        ...
        min_count: int,
        max_count: int,
        debug_img: ndarray = None) -> bool:
    h, w = symbols.shape[:2]
    x1, y1, x2, y2 = _clip_box(bbox, (h, w))

    start_y = max(0, y1 - round(unit_size / 2))
    right_bound = x2 + 1
    max_right = min(w, round(x2 + unit_size))

    while right_bound < max_right:
        cur_scan_line = note_id_map[start_y:y2, right_bound]
    ...
    left_bound = min(w, x2 + round(unit_size * 0.4))
    if left_bound >= right_bound or start_y >= y2:
        return False

    dot_region = _bin(symbols[start_y:y2, left_bound:right_bound])
    pixels = int(dot_region.sum())
    if min_count <= pixels <= max_count:
        if debug_img is not None:
            color = (255, random.randint(0, 255), random.randint(0, 255))
            cv2.rectangle(debug_img, (left_bound, start_y), (right_bound, y2), color, 1)
            msg = f"{min_count:.2f}/{pixels:.2f}/{max_count:.2f}"
            cv2.putText(debug_img, msg, (x1, y2 + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 1)
        return True
...
def parse_dot(min_area_ratio: float = 0.08, max_area_ratio: float = 0.2) -> None:
    ...
    clefs_accidentals = layers.get_layer('clefs_keys_pred')
    notes = layers.get_layer('notes')
    note_id_map = layers.get_layer('note_id')

    # Post-process necessary variables
    no_stem = _bin(symbols - stems - clefs_accidentals)
    ker = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (4, 4))
    no_stem = cv2.dilate(cv2.erode(no_stem, ker), ker)
    ...
    for group in groups:
        ...
        for nid in nids:
            bbox = notes[nid].bbox
            scan_box = (bbox[0], bbox[1], max(bbox[2], gbox[2]), bbox[3])
            has_dot = scan_dot(no_stem, note_id_map, scan_box, unit_size, min_count, max_count, debug_img=dot_img)
            dots.append(has_dot)
            notes[nid].has_dot = has_dot

        all_same = all(dots) or not any(dots)  # Notes all have dot or not
        if group.stem_up is not None and not all_same:
...
def parse_beams(
        ...
        min_width_ratio: float = 0.2) -> Tuple[ndarray, List[RotatedRect], ndarray]:
    # Fetch parameters
    symbols = layers.get_layer('symbols_pred')
    ...
    beams = polish_symbols(staff_pred, symbols, stems, clefs_accidentals, group_map)
    ...
    poly_map = np.ones((h, w, 3), dtype=np.uint8) * 255
    idx = np.where(beams > 0)
    poly_map[idx[0], idx[1]] = 0
    invalid_map = np.zeros((h, w), dtype=np.uint8)  # Used to eliminate elements in 'symbols' of later stage.
    ...
    for rbox in rboxes:  # type: ignore
        # Get the contour of the rotated box
        cnt = cv2.boxPoints(rbox).astype(np.int32)
        if (cnt < 0).any():
            # Check there is no negative points that could further cause overflow while converting to unsigned int.
            continue
        ...
        if area < min_area:
            ...
            cv2.fillPoly(invalid_map, [cnt], color=1)
            continue

        # Get pixels inside contour and keep only actual beam pixels
        mask = _contour_mask(beams.shape, cnt)
        yi, xi = np.where(mask > 0)
        meta_idx = np.where(beams[yi, xi] > 0)[0]
        ryi = yi[meta_idx]
        rxi = xi[meta_idx]
    ...
    invalid_map = invalid_map.astype(np.uint8)
...
def scan_beam_flag(
    ...
    if debug_img is not None:
        cv2.line(debug_img, (start_x, start_y), (end_x, start_y), (42, 110, 200), 2, cv2.LINE_8)
        cv2.line(debug_img, (start_x, end_y), (end_x, end_y), (42, 110, 200), 2, cv2.LINE_8)
    ...
    for x in range(start_x, end_x):
        col = poly_map[start_y:end_y, x].astype(np.uint8)
        if len(col) == 0:
            counter.append(0)
            continue
        ...
        for val in col:
            if val > 0:
                ...
                if run_len >= min_width:
                    beam_count += math.ceil(run_len / max_width)
                run_len = 0
                in_run = False

        if in_run and run_len >= min_width:
            beam_count += math.ceil(run_len / max_width)

        counter.append(beam_count)

    # Summarize counts
    stat = sorted(Counter(counter).items(), key=lambda s: s[0], reverse=True)

    # At least there are such amount agreed with that there are this number of beams/flags.
    accum = 0
...
def parse_inner_groups(poly_map, group, set_box, note_type_map, half_scan_width, threshold=0.1, debug_img=None):
    # Fetch parameters
    notes = layers.get_layer('notes')
...
def parse_rhythm(beam_map: ndarray, map_info: Dict[int, Dict[str, Any]], agree_th: float = 0.15) -> ndarray:
    ...
    for gid in range(len(groups)):
        ...
        if gid not in rev_map_info:
            # Has no beams or flags attached to this group, thus could only be quarter notes
            for nid in group.note_ids:
...
def rhythm_extract(
    ...
    agree_th: float = 0.15) -> Tuple[ndarray, List[RotatedRect]]:
