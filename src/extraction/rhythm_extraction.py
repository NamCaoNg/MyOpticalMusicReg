...
def parse_dot(min_area_ratio: float = 0.08, max_area_ratio: float = 0.2) -> None:
    ...
    no_stem = np.where(symbols-stems-clefs_accidentals>0, 1, 0)
    ...
    for group in groups:
        ...
        for nid in nids:
            ...
            has_dot = scan_dot(no_stem, note_id_map, bbox, unit_size, min_count, max_count, dot_img)
            dots.append(has_dot)
...
def parse_beams(
    ...
    for box_idx, rbox in enumerate(rboxes):  # type: ignore
        # Used to find indexes of contour areas later. Must be check before any 'continue' statement.
...
def parse_inner_groups(poly_map, group, set_box, note_type_map, half_scan_width, beam_img, threshold=0.1):
    # Fetch parameters
    notes = layers.get_layer('notes')
