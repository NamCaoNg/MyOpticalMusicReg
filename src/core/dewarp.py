...
from src.utils.morph import morph_open
...
def build_grid_group(grid_map: ndarray, grids: List[Grid]) -> Tuple[ndarray, List[GridGroup]]:
    ...
    for i in range(feat_num):
        ...
        box = (min(lbox[0], rbox[0]), min(lbox[1], rbox[1]), max(lbox[2], rbox[2]), max(lbox[3], rbox[3]))
        gg = GridGroup()
        gg.reg_id = i + 1
        gg.gids = gids
        gg.bbox = box
...
if __name__ == "__main__":
    f_name = "wind2"
    img_path = f"../data/{f_name}.jpg"

    img_path = ".\data\my_data\Be_the_one01.jpg"
    ...
    # 1234567
    out_path = os.path.join(parent_dir, f_name + "_dewarped.jpg")
    mix_path = os.path.join(parent_dir, f_name + "_compare.jpg")
